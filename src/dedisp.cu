...
dedisp_error update_scrunch_list(dedisp_plan plan) {
    ...
    dm_utils::generate_scrunch_list(plan->scrunch_list, plan->dt, plan->dm_list,
                                    plan->nchans, plan->f0, plan->df,
                                    plan->pulse_width, plan->scrunch_tol);
...
dedisp_error dedisp_generate_dm_list(dedisp_plan plan,
    ...
    plan->dm_list.clear();
    dm_utils::generate_dm_list(plan->dm_list, dm_start, dm_end, plan->dt, ti,
                               plan->f0, plan->df, plan->nchans, tol);
    ...
    plan->max_delay = dedisp_size(plan->dm_list[plan->dm_count - 1]
                                      * plan->delay_table[plan->nchans - 1]
                                  + 0.5);
...
dedisp_error dedisp_set_device(int device_idx) {
    ...
    if (cudaErrorInvalidDevice == error || cudaErrorInvalidValue == error)
...
dedisp_error dedisp_set_killmask(dedisp_plan plan,
    ...
    if (0 != killmask) {
        ...
        std::fill(plan->killmask.begin(), plan->killmask.end(),
                  (dedisp_bool) true);
...
dedisp_size dedisp_get_dm_delay(const dedisp_plan plan, int dm_trial) {
    ...
    return (plan->dm_list[dm_trial] * plan->delay_table[plan->nchans - 1]
            + 0.5);
}
...
dedisp_bool dedisp_using_adaptive_dt(const dedisp_plan plan) {
    if (!plan) {
        throw_getter_error(DEDISP_INVALID_PLAN, false);
    }
...
dedisp_error dedisp_execute_guru(const dedisp_plan plan,
    ...
    dedisp_size out_bytes_per_sample
        = 32 / (sizeof(dedisp_byte) * BITS_PER_BYTE);
    ...
    if (in_nbits != 1 && in_nbits != 2 && in_nbits != 4 && in_nbits != 8
        && in_nbits != 16 && in_nbits != 32) {
    ...
    */
    ...
    dedisp_size nsamps_padded_gulp_max
        = div_up(nsamps_computed_gulp_max, (dedisp_size)DEDISP_SAMPS_PER_THREAD)
    ...
    if (using_host_memory) {
        try {
            d_out_buf.resize(out_count_gulp_max);
        } catch (...) {
            throw_error(DEDISP_MEM_ALLOC_FAILED);
        }
    ...
    try {
        d_transposed_buf.resize(in_count_padded_gulp_max /* * 2 */);
    } catch (...) {
        throw_error(DEDISP_MEM_ALLOC_FAILED);
    }
    ...
    // Note: * 2 here is for the time-scrunched copies of the data
    try {
        d_unpacked_buf.resize(unpacked_count_padded_gulp_max * 2);
    } catch (...) {
        throw_error(DEDISP_MEM_ALLOC_FAILED);
    }
    ...
    // Gulp loop
    for (dedisp_size gulp_samp_idx = 0; gulp_samp_idx < nsamps_computed;
         gulp_samp_idx += nsamps_computed_gulp_max) {

        dedisp_size nsamps_computed_gulp
            = min(nsamps_computed_gulp_max, nsamps_computed - gulp_samp_idx);
        dedisp_size nsamps_gulp = nsamps_computed_gulp + plan->max_delay;
        dedisp_size nsamps_padded_gulp
            = div_up(nsamps_computed_gulp, (dedisp_size)DEDISP_SAMPS_PER_THREAD)
                  * DEDISP_SAMPS_PER_THREAD
              + plan->max_delay;
        ...
        if (using_host_memory) {
            ...
            if (!copy_host_to_device_2d(
                    (dedisp_byte*)d_in, in_buf_stride_words * BYTES_PER_WORD,
                    in + gulp_samp_idx * in_stride, in_stride,
                    nchan_words * BYTES_PER_WORD, nsamps_gulp)) {
                throw_error(DEDISP_MEM_COPY_FAILED);
            }
        } else if (!friendly_in_stride) {
            // Device pointers with unfriendly stride
            if (!copy_device_to_device_2d(
                    (dedisp_byte*)d_in, in_buf_stride_words * BYTES_PER_WORD,
                    in + gulp_samp_idx * in_stride, in_stride,
                    nchan_words * BYTES_PER_WORD, nsamps_gulp)) {
                throw_error(DEDISP_MEM_COPY_FAILED);
        ...
        unpack(d_transposed, nsamps_padded_gulp, nchan_words, d_unpacked,
        ...
        if (plan->scrunching_enabled) {
            ...
            for (dedisp_size s = 2; s <= max_scrunch; s *= 2) {
                // TODO: Need to pass in stride and count? I.e.,
                // nsamps_padded/computed_gulp
                ...
                scrunch_x2(&d_unpacked[scrunch_in_offset],
                           nsamps_padded_gulp / (s / 2), unpacked_nchan_words,
                           unpacked_in_nbits, &d_unpacked[scrunch_out_offset]);
        ...
        if (plan->scrunching_enabled) {
            ...
            for (dedisp_size s = 0; s < dm_count; ++s) {
                ...
                if (s + 1 == dm_count
                    || plan->scrunch_list[s + 1] != cur_scrunch) {
                    ...
                    // Note: This has the effect of increasing dt in the delay
                    // eqn
                    ...
                    dedisperse.dedisperse(
                        ...
                        nsamps_computed_gulp / cur_scrunch, unpacked_in_nbits,
                        plan->nchans, 1, d_scrunched_dm_list_ptr, scrunch_count,
                        1, d_out + scrunch_start * out_stride_gulp_bytes,
                        out_stride_gulp_samples, 0);
            ...
            dedisperse.dedisperse(
                ...
                unpacked_in_nbits, plan->nchans, 1,
                thrust::raw_pointer_cast(&plan->d_dm_list[first_dm_idx]),
                dm_count, 1, d_out, out_stride_gulp_samples, 0);
        }
        ...
        if (using_host_memory) {
            dedisp_size gulp_samp_byte_idx
                = gulp_samp_idx * out_bytes_per_sample;
            dedisp_size nsamp_bytes_computed_gulp
                = nsamps_computed_gulp * out_bytes_per_sample;
#ifdef DEDISP_BENCHMARK
            ...
            if (plan->scrunching_enabled) {
                ...
                for (dedisp_size s = 0; s < dm_count; ++s) {
                    ...
                    if (s + 1 == dm_count
                        ...
                        dedisp_float* src = d_out + scrunch_start * src_stride;
                        dedisp_float* dst
                            = (out + scrunch_start * out_stride
                               + gulp_samp_byte_idx / cur_scrunch);
                        dedisp_size width
                            = nsamp_bytes_computed_gulp / cur_scrunch;
    ...
    float total_time = copy_to_timer.getTime() + copy_from_timer.getTime()
                       + transpose_timer.getTime() + kernel_timer.getTime();
    cout << "Total time:     " << total_time << endl;

    // Append the timing results to a log file
    std::ofstream perf_file("perf.log", std::ios::app);
    perf_file << copy_to_timer.getTime() << "\t" << copy_from_timer.getTime()
              << "\t" << transpose_timer.getTime() << "\t"
              << kernel_timer.getTime() << "\t" << total_time << endl;
    perf_file.close();
#endif
...
dedisp_error dedisp_execute_adv(const dedisp_plan plan,
    ...
    return dedisp_execute_guru(plan, nsamps, in, in_nbits, in_stride, out,
...
dedisp_error dedisp_execute(const dedisp_plan plan,
    ...
    return dedisp_execute_adv(plan, nsamps, in, in_nbits, in_stride, out,
                              out_stride, flags);
}
